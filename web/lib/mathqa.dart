...
class MathQAPage extends StatefulWidget {
  const MathQAPage({super.key});

  @override
  State<MathQAPage> createState() => _MathQAPageState();
}

class _MathQAPageState extends State<MathQAPage> {
                ...
                child: ConstrainedBox(
                  ...
                  child: Theme(
                    ...
                    child: LlmChatView(
                      ...
                      welcomeMessage: '你好，我是数学助手，可以帮你解答各种数学问题。',
                      suggestions: const [
                        '帮我讲一下勾股定理',
                        '怎么求一元二次方程的解？',
                        '解释一下导数的几何意义',
...
class BackendLlmProvider extends LlmProvider {
  ...
  Stream<String> generateStream(
    ...
    final request = http.Request('POST', uri)
      ...
      ..body = jsonEncode(_buildRequestBody(prompt, attachments));
  ...
  Map<String, dynamic> _buildRequestBody(
    ...
    for (final attachment in attachments) {
      final dynamic a = attachment;

      // 尽量从 Attachment 中提取可读的名称和文本内容；
      // 若获取失败则使用 toString() 作为回退，避免影响主流程。
      String name = 'attachment';
      try {
        final dynamic n = a.name;
        if (n != null) {
          name = '$n';
        }
      } catch (_) {
      ...
      if (content == null) {
        try {
          final dynamic b = a.bytes;
          if (b is List<int> && b.isNotEmpty) {
            content = utf8.decode(b, allowMalformed: true);
          }
        } catch (_) {
          // ignore
        }
    ...
    final Map<String, dynamic> body = <String, dynamic>{'prompt': prompt};
    if (files.isNotEmpty) {
      body['files'] = files;
    }
